...
async def settings(bot: Client, msg):
    ...
    if not await chat_exists(msg.chat.id):
        await msg.reply("Please add a force subscribe chat by  using /fsub to use me.")
        return
    ...
    await msg.reply(
        "**Settings** \n\n"
        "1) Choose what to do with them who haven't joined the force subscribe chat. Default is Mute.\n"
        ...
        "3) Choose to allow admins to change fsub chat and settings or not. Default is 'Allow Only Owner'.",
